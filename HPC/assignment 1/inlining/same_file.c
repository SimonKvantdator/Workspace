...
int main() {
	...
	for (int i = 0; i < NBR_BENCHMARK_ITERATIONS / 10; i++) {
		...
		for (int j = 0; j < ARRAY_LENGTH; j++) {
			mul_cpx(&as_re[j], &as_im[j], &bs_re[j], &bs_im[j], &cs_re[j], &cs_im[j]);
		}
	}
...
// Multiplying 30000 complex numbers took an average (over 1000000 benchmark iterations) of 1.915042e-03 s
