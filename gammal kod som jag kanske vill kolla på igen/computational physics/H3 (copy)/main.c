...
void task1() {
	...
	double d = 0.5; // wavepacket width [Å]
	double x; // Wavefunction's position coordinate [Å]
	double x0 = 0.0; // Mean position [Å]
	double x_max = 30.0; // x goes from x0 - x_max to x0 + x_max [Å]
	...
	double p0 = sqrt(2 * hydrogen_mass * 0.1); // [eV fs / Å] mean momentum obtained from solving p0^2 / 2m = 0.1 eV
...
void task2() {
	...
	double timestep = 0.8; // [fs]
	int nbr_timesteps = 200;
	...
	int x_list_length = ipow(2, 12); // List length 2^n - 1 optimal for FFT
	double x_list[x_list_length]; // List of positions [Å]
	double V_list[x_list_length]; // List of potential energies [eV]
	double complex psi_x_list[x_list_length]; // psi(x) [Å^(-1/2)]
	double *real_psi_x_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Real part of psi(x), real_psi_x_list[x_index + x_list_length * time_index], [Å^(-1/2)]
	double *imag_psi_x_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Imaginary part of psi(x), imag_psi_x_list[x_index + x_list_length * time_index], [Å^(-1/2)]
	double dx = 2 * x_max / x_list_length;
	double normalization = 2 * x_max / (x_list_length * sqrt(2 * PI * hbar)); // For normalizing psi(p)

	double p0 = sqrt(2 * hydrogen_mass * 0.1); // [eV fs / Å] mean momentum obtained from solving p0^2 / 2m = 0.1 eV
	double p_list[x_list_length]; // List of momenta [eV fs / Å]
	double complex psi_p_list[x_list_length]; // psi(p) [(eV fs / Å)^(-1/2)]
	double *real_psi_p_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Real part of psi(p), real_psi_p_list[x_index + x_list_length * time_index], [(eV fs / Å)^(-1/2)]
	double *imag_psi_p_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Imaginary part of psi(p), imag_psi_x_list[x_index + x_list_length * time_index], [(eV fs / Å)^(-1/2)]
...
void task3(double alpha, double T0) {
	...
	double timestep = 0.8; // [fs]
	int nbr_timesteps = 400;

	double d = 0.5; // Wavepacket width [Å]
	double V0 = 0.1; // [eV]
	double x; // Wavefunction's position coordinate [Å]
	double x0 = -3.0; // Mean position [Å]
	double x_max = 30.0; // x goes from x0 - x_max to x0 + x_max [Å]
	...
	double p0 = sqrt(2.0 * hydrogen_mass * T0); // Mean momentum obtained from solving p0^2 / 2m = 0.1 eV, [eV fs / Å]
	double p_list[x_list_length]; // List of momenta [eV fs / Å]
	double complex psi_p_list[x_list_length]; // psi(p) [(eV fs / Å)^(-1/2)]
	double *real_psi_p_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Real part of psi(p), real_psi_p_list[x_index + x_list_length * time_index], [(eV fs / Å)^(-1/2)]
	double *imag_psi_p_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Imaginary part of psi(p), imag_psi_x_list[x_index + x_list_length * time_index], [(eV fs / Å)^(-1/2)]
...
void task5(double T0) {
	...
	double d = hbar / sqrt(4 * hydrogen_mass * 0.1 * T0); // Wavepacket width [Å]
	// double V0 = 0.1; // [eV]
	double x; // Wavefunction's position coordinate [Å]
	double x0 = 0.0; // Mean position [Å]
	double x_max = 30.0; // x goes from x0 - x_max to x0 + x_max [Å]
	...
	int x_list_length = ipow(2, 12); // List length 2^n - 1 optimal for FFT
	double x_list[x_list_length]; // List of positions [Å]
	double complex psi1_x_list[x_list_length]; // psi1(x) [Å^(-1/2)]
	double complex psi2_x_list[x_list_length]; // psi2(x) [Å^(-1/2)]
	double *real_psi1_x_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Real part of psi(x), real_psi_x_list[x_index + x_list_length * time_index], [Å^(-1/2)]
	double *real_psi2_x_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Real part of psi(x), real_psi_x_list[x_index + x_list_length * time_index], [Å^(-1/2)]
	double *imag_psi1_x_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Imaginary part of psi(x), imag_psi_x_list[x_index + x_list_length * time_index], [Å^(-1/2)]
	double *imag_psi2_x_list = malloc(x_list_length * nbr_timesteps * sizeof(double)); // Imaginary part of psi(x), imag_psi_x_list[x_index + x_list_length * time_index], [Å^(-1/2)]
	double dx = 2.0 * x_max / x_list_length;
	double normalization = 2.0 * x_max / (x_list_length * sqrt(2.0 * PI * hbar)); // For normalizing psi(p)

	double p0 = sqrt(2.0 * hydrogen_mass * T0); // Mean momentum obtained from solving p0^2 / 2m = 0.1 eV, [eV fs / Å]
	double p_list[x_list_length]; // List of momenta [eV fs / Å]
	double complex psi1_p_list[x_list_length]; // psi(p) [(eV fs / Å)^(-1/2)]
	double complex psi2_p_list[x_list_length]; // psi(p) [(eV fs / Å)^(-1/2)]
...
int main() {
	...
	// double alphas[] = {0.5, 0.5, 0.5, 2.0, 2.0, 2.0}; // [Å]
